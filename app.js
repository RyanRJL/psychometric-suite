...
function reportOrdinal(value){
  ...
  // Deliberately open-ended rather than bounded at 0 and 100. A percentile rank
  // can never be exactly 0 or 100, so anything at or below 1 reads "<1st" and
  // anything at or above 99 reads ">99th". The previous strict guards
  // (raw > 0 && raw < 1, raw > 99 && raw < 100) let a value that had rounded to
  // 0.00 or 100.00 fall through to the generic suffix branch, so generated
  // report text asserted "0th percentile" / "100th percentile" — impossible
  // ranks. A user typing 0 or 100 into the percentile field hits this too.
  if (raw < 1)  return '&lt;1<sup>st</sup>';
  if (raw > 99) return '&gt;99<sup>th</sup>';
