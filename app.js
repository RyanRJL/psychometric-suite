...
function navigateTo(target, opts){
  ...
  const swap = () => {
    ...
    /* Profile Analysis reads its scores from Score Tables and re-renders when one
       changes — but arriving on the page is its own event, and a page that is
       stale on arrival is the whole failure this belt-and-braces call closes. It
       is cheap: the simulation is cached on the selection. */
    if (target === 'profile' && typeof renderProfile === 'function') renderProfile();
    /* Re-fit the scrolling table boxes. A box on a hidden page measures 0
...
/* THE HOOK BELONGS HERE, NOT IN renderBattery. A score keystroke updates the
   row IN PLACE — so the caret is not thrown away — and ends in renderBatteryApa,
   never in renderBattery, which runs only when rows are added or removed. The
   hook shipped in renderBattery and so never fired on a typed score: rows loaded
   from Quick Add synced while still blank, the scores were then typed, and the
   Profile page went on showing "No WAIS-IV scores yet".

   renderBattery ends by calling this too, so one hook covers both paths.
   Profile Analysis holds no scores of its own — it reads this table — so this is
   the only thing that can change what it shows. The module coalesces a burst of
...
function renderBatteryApa(){
  ...
  /* Rows come and go through renderBattery, which ends here — same single
     hook the Profile page uses, for the same reason. */
...
function pvtInstrumentLineHtml(tab){
  ...
  return `<span class="pvt-derived"><span class="pvt-derived-label">Derived on</span> ${i.derived}${
    i.unspecified ? ' <span class="pvt-derived-gap">(form or edition not recorded here)</span>' : ''}</span>${
...
/* ============================================================
   ...
   tables so the table scrolls rather than the page. A fixed cap cannot be
   right on every machine: 560px leaves a laptop with dead space below the
   fold on a tall monitor and pushes the add-row buttons off-screen on a
   short one. So the cap is measured, per box, against the actual window.

   The sum is deliberately RELATIVE rather than modelled. Working out the
   margins, paddings and control rows that sit below each box would be a
   second copy of the layout that drifts from the real one. Instead: read
   where the content below the box currently ENDS, see how far past the
   fold that is, and take exactly that much off the box. Changing a box's
   max-height moves everything below it by the same amount and nothing
   else, so one pass lands exactly - no iteration, no settling.

   BOTH HALVES OF THE ZOOM SPLIT ARE HERE, which is the thing to get right:
   every measured rect is VISUAL px (body{zoom:0.9} has already been
   applied), and style.maxHeight is LAYOUT px (the browser applies the zoom
   itself). The subtraction happens in visual px and the result is divided
   by pageZoomFactor() once, at the point of writing. Mixing them is silent
   and systematic - see the popover post-mortem in CLAUDE.md.

   The floor matters as much as the fit. On a short window the honest
   answer is that the table cannot fit, and a 40px sliver would be worse
...
/* `var`, DELIBERATELY, and the only three in this neighbourhood. This block
   ...
   renderBatteryApa, which schedules a fit — is called much earlier in the
   init sequence. A `let` is in the temporal dead zone until its own line
   runs, so the app threw `Cannot access 'tableViewportPending' before
   initialization` on boot and every top-level statement after it stopped.
   That is the init-tail hazard in CLAUDE.md wearing a new hat, and it was
   caught by driving the real page rather than by node --check. `var` hoists
   as undefined, which every reader here already handles. */
var tableViewportRO = null;
var tableViewportPanelMO = null;
var tableViewportPending = null;

function fitTableViewports(){
  const z = typeof pageZoomFactor === 'function' ? pageZoomFactor() : 1;

  document.querySelectorAll('.table-viewport').forEach(box => {
...
function observeTableViewportTables(){
  if (!tableViewportRO) return;
  /* The TABLE is observed, never the box: the box's max-height is what this
     module writes, so observing it would feed its own output straight back
     in. A table's height changes only when rows are added or removed. */
  document.querySelectorAll('.table-viewport table').forEach(table => {
    if (table.dataset.tvObserved) return;
    table.dataset.tvObserved = '1';
    tableViewportRO.observe(table);
  });
}

function observeChangeMethodPanels(){
  if (!tableViewportPanelMO) return;
  /* Switching method tabs shows a different panel without any navigation and
     without touching a table, so neither navigateTo nor the ResizeObserver
     fires. The panel's own class is the only thing that changes — the same
     signal watchMethodPanels reads for the consent card. */
  document.querySelectorAll('.change-method-panel').forEach(panel => {
    if (panel.dataset.tvPanelObserved) return;
    panel.dataset.tvPanelObserved = '1';
    tableViewportPanelMO.observe(panel, { attributes:true, attributeFilter:['class'] });
  });
}

/* RE-SCANNED ON EVERY PASS, NOT ONCE AT INIT. app.js is a plain script at the
   top of the document and the five Change Analysis panels are built by an
   inline script BELOW it, so at the moment this module initialises there is
   not a single .change-method-panel or wrapped table in the DOM. A one-shot
   scan found nothing, attached nothing, and left those four tables on the
   stylesheet's fallback cap for ever. Both scans are idempotent (a dataset
   flag per element), so re-running them costs a querySelectorAll. */
function refreshTableViewportFit(){
  observeTableViewportTables();
  observeChangeMethodPanels();
  fitTableViewports();
}

function setupTableViewportFit(){
  if (typeof ResizeObserver === 'function' && !tableViewportRO){
    tableViewportRO = new ResizeObserver(() => scheduleTableViewportFit());
  }
  if (typeof MutationObserver === 'function' && !tableViewportPanelMO){
    tableViewportPanelMO = new MutationObserver(() => scheduleTableViewportFit());
  }
  window.addEventListener('resize', scheduleTableViewportFit);
  /* One pass after everything below app.js has run and laid out. */
  window.addEventListener('load', scheduleTableViewportFit);
  scheduleTableViewportFit();
}

function scheduleTableViewportFit(){
  if (tableViewportPending) cancelAnimationFrame(tableViewportPending);
  tableViewportPending = requestAnimationFrame(() => {
    tableViewportPending = null;
    refreshTableViewportFit();
  });
}
window.fitTableViewports = fitTableViewports;
