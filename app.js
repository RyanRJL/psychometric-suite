...
/* Where the failing side starts, in value units. Integer scales, so the
   boundary sits on the half-integer between the last passing and the first
   failing score — exact, not eyeballed. */
function pvtFailBoundary(op, cut){
  if (op === 'lt')  return cut - 0.5;   // fail if score <  cut
  if (op === 'lte') return cut + 0.5;   // fail if score <= cut
  if (op === 'gt')  return cut + 0.5;   // fail if score >  cut
  return cut - 0.5;                     // 'gte': fail if score >= cut
}
function pvtMeterHtml(o){
  if (!Number.isFinite(o.value) || !Number.isFinite(o.min) || !Number.isFinite(o.max)) return '';
  const span = o.max - o.min;
  if (!(span > 0)) return '';
  const pct = v => Math.max(0, Math.min(100, ((v - o.min) / span) * 100));
  const b = pctClamp(pct(pvtFailBoundary(o.op, o.cut)));
  const failLow = (o.op === 'lt' || o.op === 'lte');
  const zone = failLow
    ? `left:0;width:${b.toFixed(2)}%`
    : `left:${b.toFixed(2)}%;right:0`;
  const markPct = pct(o.value);
  const label = `${o.label}: ${o.value} on a ${o.min}–${o.max} scale; ${
    failLow ? 'scores at or below' : 'scores at or above'} the cut-off fall in the shaded zone`;
  return `<span class="pvt-meter" role="img" aria-label="${escapeAttr(label)}">
      <span class="pvt-meter-zone" style="${zone}"></span>
      <span class="pvt-meter-mark${o.state === 'pass' ? '' : ' is-flagged'}" style="left:${markPct.toFixed(2)}%"></span>
    </span>`;
}
...
function pvtIndexRowHtml(o){
  ...
  return `<div class="pvt-index">
    <div class="pvt-index-main">
      ...
      ${o.state === 'na' ? '' : pvtMeterHtml(o)}
      ${o.meta ? `<div class="pvt-index-meta">${o.meta}</div>` : ''}
    </div>
    <span class="pvt-flag is-${chipState}">${word}</span>
  </div>`;
}
...
function renderPvtEs(){
  ...
  out.innerHTML = pvtReadoutHtml([{
    ...
    min: -42, max: 36, cut: PVT_ES.cutoff, op: 'lt',
...
function renderPvtDs(){
  ...
  if (s.diff !== undefined) rows.push({
    ...
    /* Both scaled scores run 1-19, so the difference runs -18 to 18. */
    min: -18, max: 18, cut: PVT_DS.vocabDiffCutoff, op: 'gte',
  ...
  if (s.lsf !== undefined){
    if (s.lsfState === 'ok') rows.push({
      ...
      min: 0, max: 9, cut: PVT_DS.lsfCutoff, op: 'lte',
  ...
  if (s.lsb !== undefined) rows.push({
    ...
    min: 0, max: 8, cut: PVT_DS.lsbCutoff, op: 'lte',
...
function renderPvtRey(){
  ...
  const rows = [{
    ...
    min: 0, max: 15, cut: PVT_REY15.recallCutoff, op: 'lt',
  ...
  if (s.combo !== undefined){
    /* Domain is the arithmetic range of the formula's own inputs:
       recall 0-15 plus (recognition 0-15 minus false positives 0-15). */
    rows.push({
      label: 'Combination score', value: s.combo, state: s.comboFail ? 'fail' : 'pass',
      min: -15, max: 30, cut: PVT_REY15.comboCutoff, op: 'lt',
...
function renderPvtTomm(){
  ...
  out.innerHTML = pvtReadoutHtml(s.rows.map(r => ({
    ...
    min: 0, max: 50, cut: r.cutoff.cut, op: 'lt',
