...
(function(){
  ...
  function buildInlineControlBar(method){
    ...
    `;
    /* MOUNT ABOVE THE SCROLL BOX, NEVER INSIDE IT. The Change Analysis tables
       are wrapped in .change-table-scroll.table-viewport (index.html), so
       `table.parentNode` is that wrapper - inserting there put the Quick Add
       bar inside a clipping scroll container and its dropdown was cut off at
       the box edge, which is what a scroll container is for. The bar is a
       page control anyway: it must stay put while the rows scroll under it. */
    const barAnchor = table.parentNode.classList.contains('table-viewport')
      ? table.parentNode
      : table;
  ...
  function buildBatteryInlineBar(){
    ...
    bar.innerHTML = `
      ...
      <!-- THE TOOLTIPS ARE THE CONTROL'S ONLY EXPLANATION, so they carry the
           whole answer rather than a restatement of the button label. Written
           after a clinician read the two words and could not tell what either
           did — "Published/Corrected" is meaningless without knowing that only
           the r changes, and that it changes for almost no measures.

           Newlines via &#10;, which native title honours. Kept as title rather
           than a styled tooltip deliberately: this bar's sections have caused
           enough trouble already, and title costs nothing in cascade risk,
           needs no positioning maths (see the body{zoom:0.9} note in
           CLAUDE.md), and is read out by screen readers. -->
      <div class="ds-inline-bar-section ds-inline-bar-cibasis" hidden>
        <span class="ds-inline-bar-label" title="Which reliability coefficient (r) the confidence intervals are built from.&#10;&#10;Every interval is score ± z × SD × √(1 − r), so this setting changes how wide the intervals are and nothing else. In practice it only affects D-KEFS and D-KEFS Advanced measures.">Reliability</span>
        <div class="ds-inline-bar-toggle" role="radiogroup" aria-label="Reliability coefficient basis">
          <button type="button" class="ds-inline-bar-toggle-btn is-active" data-bat-ci-basis="published" role="radio" aria-checked="true"
            title="Uses each manual's own r, exactly as that manual uses it, so your intervals match the ones printed in the manual and anyone checking your report against the source will find agreement.&#10;&#10;The default. Leave it here unless you have a particular reason not to.">Published</button>
          <button type="button" class="ds-inline-bar-toggle-btn" data-bat-ci-basis="corrected" role="radio" aria-checked="false"
            title="Adjusts the manual's r to estimate what it would have been in the normative sample. Statistically tidier pairing, but the resulting number appears in no manual, so a reader checking your interval against the source will not be able to reproduce it.&#10;&#10;corrected r = 1 − (retest-sample SD² ÷ normative SD²) × (1 − r)&#10;&#10;Only D-KEFS and D-KEFS Advanced are affected, because every other manual already publishes a coefficient measured on its own normative sample, so there is nothing to adjust. That leaves 46 measures. At 95% it moves 13 printed margins (9 wider, 4 narrower) by at most 2 scaled-score points.&#10;&#10;If you use it, the note under the exported table says so.">Corrected</button>
        </div>
      </div>
