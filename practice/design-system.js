...
(function(){
  ...
  document.addEventListener('click', e => {
    ...
    setTimeout(() => chip.classList.remove('rb-chip-orbit'), 1500);  /* matches CSS 1.4s + tiny buffer */
  }, true);
  ...
  // overlay (no HTML edits required, and easy to revert by deleting this
  // block). Also builds a minimalistic model-availability tick list inside
  // the aside that updates live as the user fills in fields.
  // ============================================================================
  ...
  function restructurePremorbid(){
    ...
    // Insert layout where the tabs currently sit
    tabs.parentNode.insertBefore(layout, tabs);

    // Move inputs panel into aside
    aside.appendChild(inputsPanel);

    // Shorten field labels for the compact aside layout. Map by input id so
    // we don't depend on the source label text. Idempotent — safe if already
    // applied (the new text just gets re-set to itself).
    const labelMap = {
      'pre-topf':  'ToPF Raw',
      'pre-vc':    'Vocabulary',
    ...
    Object.keys(labelMap).forEach(id => {
      ...
      if (!lbl) return;
      // Preserve the .pre-label-sub child if it exists (it's hidden by CSS but
      // some upstream code may read it). Just replace the leading text node.
      const sub = lbl.querySelector('.pre-label-sub');
      lbl.textContent = labelMap[id];
      if (sub) lbl.appendChild(sub);
    });
    // Inputs tab content wrapper is now empty — hide it
    if (inputsTab) inputsTab.style.display = 'none';

    // Move tabs + remaining tab contents into main
    ...
    // If the Inputs tab was the active one, switch to Estimates
    ...
    if (inputsTabBtn && inputsTabBtn.classList.contains('active') && estimatesBtn){
      // Click programmatically so app.js's tab handler runs
      estimatesBtn.click();
    }
    // Remove the Inputs tab outright — CSS hide may be losing a specificity
    // battle in some browsers, so just yank it from the DOM.
    if (inputsTabBtn && inputsTabBtn.parentNode){
      inputsTabBtn.parentNode.removeChild(inputsTabBtn);
    }

    // Build the tick list and append to aside
